...
function AssessmentPage() {
  ...
  const fetchQuestions = (forceReload = false) => {
    ...
    if (cachedQuestions && !forceReload && !reloaded) {
      ...
      try {
        ...
        axios
          .get(`https://placement-platform-production-4dbe.up.railway.app/api/questions/level${level}/${domain}/random`)
          .then((res) => {
  ...
  const submitAssessment = async (auto = false) => {
    ...
    try {
      ...
      const response = await axios.post("https://placement-platform-production-4dbe.up.railway.app/api/assessment/submit", {
